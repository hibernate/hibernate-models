...
import static org.hibernate.models.accessor.asm.impl.HibernateAccessorAsmUtils.emitBox;
import static org.hibernate.models.accessor.asm.impl.HibernateAccessorAsmUtils.emitIntConstant;
import static org.hibernate.models.accessor.asm.impl.HibernateAccessorAsmUtils.emitUnboxOrCast;
...
final class HibernateAccessorAsmMultiValueClassGenerator implements Opcodes {
	...
	// --- Pointcuts-aware overloads ---

	// get(Object instance) -> Object[]
	// locals: [this=0, instance=1, result=2]
	static byte[] generateReader(Class<?> targetClass, Member[] members, MultiValueAccessorPointcuts pointcuts) {
		String targetInternal = Type.getInternalName(targetClass);
		String generatedName = targetInternal + "$$HibernateAccessorMultiReader";
		...
		pointcuts.emitPreamble(mv, targetClass, 1);

		for (int i = 0; i < members.length; i++) {
			Label skipLabel = new Label();
			Label afterLabel = new Label();

			pointcuts.emitBeforeRead(mv, i, members[i], skipLabel);
			...
			// skipped - leave array slot as null (default)
	...
	// set(Object instance, Object[] values) -> void
	// locals: [this=0, instance=1, values=2]
	static byte[] generateWriter(Class<?> targetClass, Member[] members, MultiValueAccessorPointcuts pointcuts) {
		String targetInternal = Type.getInternalName(targetClass);
		String generatedName = targetInternal + "$$HibernateAccessorMultiWriter";
		...
		for (int i = 0; i < members.length; i++) {
			...
			pointcuts.emitBeforeWrite(mv, i, members[i], skipLabel);
			...
			pointcuts.emitAfterWrite(mv, i, members[i]);
			mv.visitJumpInsn(GOTO, afterLabel);
	...
	static byte[] generateBulkReader(HibernateAccessorBulkMemberAccess[] accesses, int accessorFieldCount, MultiValueAccessorPointcuts pointcuts) {
		...
		pointcuts.emitPreamble(mv, null, 1);

		for (int i = 0; i < accesses.length; i++) {
			Label skipLabel = new Label();
			Label afterLabel = new Label();

			pointcuts.emitBeforeRead(mv, i, null, skipLabel);
	...
	static byte[] generateBulkWriter(HibernateAccessorBulkMemberAccess[] accesses, int accessorFieldCount, MultiValueAccessorPointcuts pointcuts) {
		...
		for (int i = 0; i < accesses.length; i++) {
			...
			pointcuts.emitBeforeWrite(mv, i, null, skipLabel);
			...
			pointcuts.emitAfterWrite(mv, i, null);
			mv.visitJumpInsn(GOTO, afterLabel);

...
final class HibernateAccessorAsmBulkAccessorClassGenerator implements Opcodes {
	...
	static byte[] generate(Class<?> targetClass, Field[] fields, Method[] methods, Constructor<?>[] constructors) {
		...
		cw.visit(
				V17, ACC_PUBLIC | ACC_SUPER | ACC_SYNTHETIC, generatedName, null,
	...
	// locals: [this=0, instance=1, index=2]
	private static void generateReadByMethod(
			ClassWriter cw,
			String targetInternal,
			boolean isInterface,
			Method[] methods) {
		MethodVisitor mv = cw.visitMethod(
				ACC_PUBLIC, "readByMethod",
				"(Ljava/lang/Object;I)Ljava/lang/Object;", null, null
		);
		mv.visitCode();
		...
		if ( methods.length == 0 ) {
			...
			for ( int i = 0; i < methods.length; i++ ) {
				...
				if ( m.getParameterCount() != 0 || m.getReturnType() == void.class ) {
					...
					mv.visitMethodInsn(
							opcode, targetInternal, m.getName(),
	...
	// locals: [this=0, instance=1, index=2, value=3]
	private static void generateWriteByMethod(
			ClassWriter cw,
			String targetInternal,
			boolean isInterface,
			Method[] methods) {
		MethodVisitor mv = cw.visitMethod(
				ACC_PUBLIC, "writeByMethod",
				"(Ljava/lang/Object;ILjava/lang/Object;)V", null, null
		);
		mv.visitCode();
	...
	private static void emitThrow(MethodVisitor mv, String message) {
		...
		mv.visitMethodInsn(
				INVOKESPECIAL, EXCEPTION_INTERNAL, "<init>",
